...
// NAREDI TAKO, DA BOŠ IMEL UNSIGNED LONG ZA ENO STANJE - 6 triplov
...
struct trie_node{
	short value;
	map<unsigned int, trie_node*> map_children;
...
trie_node* find_node(trie_node* node, unsigned int ind){
...
trie_node* add_node(trie_node* node, unsigned int ind){
...
short check_state(vector<unsigned int> &possible_hash){
	trie_node* current = root;
	for(int i = 0; i < possible_hash.size(); i++){
		trie_node* next_node = find_node(current, possible_hash[i]); 
...
void add_state(vector<unsigned int> &minimized_possible, short dp_value){
	trie_node* current = root;
...
short f(vector<int> &possible){
	...
	short value = check_state(possible_hash);
	if(value != -1){
		return value;
	}
	...
	add_state(possible_hash, max_len);
	return max_len;
}
